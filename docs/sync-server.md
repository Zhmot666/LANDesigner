...
- **Diff** — сравнение метаданных, устройств, кабелей, VLAN локальной и серверной копии
- **Принудительный push** — перезаписать сервер (осторожно)

## Блокировки (команда)

Чтобы два инженера не перезаписывали проект одновременно:

| API | Описание |
|-----|----------|
| `GET /projects/{id}/lock` | Текущая блокировка |
| `PUT /projects/{id}/lock` | Занять/продлить (заголовки `X-Client-Id`, `X-Client-Name`) |
| `DELETE /projects/{id}/lock` | Освободить (`X-Client-Id`) |

- TTL блокировки по умолчанию: **2 часа** (продлевается при повторном acquire тем же клиентом).
- **Push** отклоняется (HTTP 423), если проект заблокирован другим `X-Client-Id`.
- В GUI: **Синхронизация → Настройки** — поле **«Ваше имя»**; при открытии привязанного проекта блокировка запрашивается автоматически.
- Список проектов на сервере показывает `lock: Имя`, если проект занят.

## Pull и резервная копия

Перед **Pull** локальный `.lanproj` автоматически копируется в снимок  
`<файл>.lanproj.snapshots/…_before_pull/` (как **Файл → Создать снимок**).
